    ...
    <section className="flex flex-col gap-4">
      <div className="mq-sechead">
        <div>
          ...
          <p className="mute" style={{ fontSize: 13 }}>Reel is invite-only — bring a friend. {unused}/10 unused.</p>
        </div>
        <button
      ...
      {invites.length === 0 ? (
        <div className="card" style={{ padding: "24px", display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <p className="dim" style={{ margin: 0, fontSize: 14 }}>No invites yet — create one to share.</p>
        </div>
      ) : (
            ...
            <div key={inv.code} className="card mq-row" style={{ cursor: "default" }}>
              ...
              {inv.status === "unused" && (
                <button className="btn btn-ghost btn-sm" onClick={() => copy(inv.code)}>
                  {copied === inv.code ? <><Check size={14} />Copied</> : <><Copy size={14} />Copy link</>}
                </button>
              )}
              {inv.status !== "unused" && (
