...
import { Activity, Check, ChevronLeft, Clock, Eye, Heart, Play, Plus, Star, Tv } from "lucide-react";
import { z } from "zod";
...
/* Friend profile page (route /friend/:id). rpc_friend_snapshot supplies the
   profile, episode counts and "watching now"; useFriendProfile adds their full
   follow list + ratings, from which the page builds the in-common, taste,
   activity and browsable-follows sections. Opening a show stacks the detail
   sheet on top via the shell's global ?title= param. */
  ...
  return (
    <div className="screen mq-page">
      ...
      <div className="card overflow-hidden">
        <div className="relative" style={{ height: 132, background: `linear-gradient(135deg, hsl(${hue} 55% 34%), hsl(${(hue + 50) % 360} 60% 18%))` }}>
          <div className="poster-sheen" />
        </div>
        <div className="px-6 pb-5" style={{ marginTop: -40 }}>
          <div className="flex items-end gap-4 flex-wrap">
            <FriendAvatar f={{ id: snap.profile.id, name: snap.profile.display_name, avatarUrl: snap.profile.avatar_url }} size={92} ring />
