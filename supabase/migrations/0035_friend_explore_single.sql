-- 0035_friend_explore_single.sql
-- Surface friend recommendations as soon as you have ONE friend: lower both
-- friend-explore aggregates from ≥2 friends to ≥1 (was 0017's threshold), and
-- expose titles.id on rpc_popular_with_friends so the Explore "With friends"
-- discover tab can add/hide those shows like the sibling Popular/Top-rated tabs.
-- Both stay security INVOKER so the 0015 friend-read RLS still applies.
...
as $$
  ...
  agg as (
    ...
    from ff group by title_id having count(*) >= 1
  )
...
create or replace function public.rpc_best_rated_by_friends()
returns jsonb
language sql
security invoker
stable
