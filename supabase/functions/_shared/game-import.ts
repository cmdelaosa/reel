 ...
 * (0104): las reglas de qué se escribe y las escrituras en sí.
 *
...
export interface Pick {
   ...
   *  evento, ver 0104— pero se parsea igual: lo que decide si se escribe es
...
export interface EntryWrite {
  ...
  /* `played_at` NO va aquí, y no por olvido: el disparador `game_progress_touch`
     (0075) es BEFORE y escribe `now()` en cuanto cambian las horas o el estado,
     así que cualquier fecha que viajara en este mismo upsert la pisaría él. La
     fecha va detrás, en su propia escritura — ver `writePlayedAt`. */
}
 ...
 *    · Del estado gana el primero que diga algo. Son decisiones de
...
/** La fecha de la última partida, escrita DESPUÉS de la biblioteca.
 *
 *  El disparador `game_progress_touch` (0075) pone `played_at = now()` en
 *  cuanto un upsert cambia las horas o el estado — y una importación cambia las
 *  horas de todo lo que toca. Sin esta segunda escritura, importar fecha HOY
 *  todos los juegos, y "vuelve a lo que estabas jugando" (domain/gameTonight)
 *  ordena por el minuto en que alguien le dio al botón.
 *
 *  Va aparte y detrás, y esa es toda la gracia: un update que solo toca
 *  `played_at` no cumple el WHEN del disparador (horas o estado distintos), así
 *  que no dispara y el valor se queda como se escribe.
 *
 *  Qué fecha poner es cosa de quien llama, porque sale de lo que cada proveedor
 *  sabe: Steam da `rtime_last_played`; Nintendo no da nada y la fecha se
 *  aprende o se deja como estaba. `at: null` es válido y escribe null — es lo
 *  que toca a un juego que entra nuevo sin que se sepa cuándo se jugó: seguir
 *  un juego no es haberlo jugado.
 *
 *  Movida aquí desde steam-sync en 0104. Dos filas del mismo juego se funden
 *  por lo mismo que en `writeEntries` (21000); gana la primera. */
export async function writePlayedAt(
  admin: SupabaseClient,
  userId: string,
  rows: { titleId: string; at: string | null }[],
): Promise<number> {
  const byTitle = new Map<string, { titleId: string; at: string | null }>();
