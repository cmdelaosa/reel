...
// Desde 0104 se llaman `game_imports` y `game_import_items`, las comparte con
...
interface Scanned {
  /** El appid, como TEXTO: desde 0104 la columna es `external_id` y la
  ...
  try {
    ...
    if (path === "/apply" && req.method === "POST") {
      ...
      const { data: run } = await admin
        ...
        .eq("id", importId)
        .eq("user_id", userId)
        // Desde 0104 la tabla la comparten dos proveedores: confirmar aquí un
...
async function resolvePending(
  ...
  /* Las filas vienen de la base, donde el appid vive como TEXTO desde 0104
  ...
  try {
    ...
    if (found.length) {
      ...
      await writePlayedAt(
        ...
        found
          .filter((i) => i.last_played_at && minutesFor(i) !== null)
          .map((i) => ({ titleId: byAppid.get(appidOf(i))!, at: i.last_played_at as string })),
      );
