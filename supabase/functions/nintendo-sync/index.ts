// nintendo-sync — Deno edge function. Traer de Nintendo las horas jugadas (0104).
//
...
import { learnedOrKept, normalizeFriendCode, parsePlayLog } from "./merge.ts";
...
interface Entry {
  ...
  source: string | null;
  followed: boolean;
  /** La última partida que ya había, para devolverla si esta importación no ha
   *  aprendido otra: el disparador de 0075 la pisa con `now()` en cuanto
   *  cambian las horas. Ver `writePlayedAt`. */
...
async function entriesOf(
  ...
  for (const part of chunk(titleIds, ID_PAGE)) {
    const { data, error } = await admin
      ...
      .select("title_id, minutes_played, minutes_source, followed, played_at")
      .eq("user_id", userId)
      .in("title_id", part);
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (path === "/link") {
      ...
      // El índice único de 0104: esa cuenta de Nintendo ya está en otro perfil
    ...
    if (path === "/apply") {
      ...
      /* Lo marcado viaja en la petición y no está guardado: la pantalla se
         ...
         la que fechar el evento (ver 0104), así que esa casilla no existe en
      ...
      const applied = await writeEntries(
        ...
        ready.map((i) => ({
          ...
          minutes: minutesToWrite(current.get(i.title_id as string), i.minutes, pickFor(i).overwrite),
          playState: pickFor(i).playState,
        })),
        "nintendo",
      );
      /* Y la fecha, DETRÁS: el disparador de 0075 acaba de poner `now()` en
         todo lo que ha cambiado de horas. Se queda la aprendida si los minutos
         han subido respecto a una cifra que ya era de Nintendo, y si no, la que
         había — null en lo que entra nuevo. Sin esto, importar veinte juegos
         los pondría los veinte arriba del todo en "A jugar". */
      await writePlayedAt(
        admin,
        userId,
        ready.map((i) => ({
          titleId: i.title_id as string,
          at: learnedOrKept(current.get(i.title_id as string), i.minutes),
        })),
      );
    ...
    if (path === "/refresh") {
      ...
      const updated = await writeEntries(
        admin,
        userId,
        rows.map(({ g, titleId }) => ({ titleId, minutes: g.minutes, playState: null })),
        "nintendo",
      );
      // La fecha detrás, por lo mismo que en /apply.
      await writePlayedAt(
        admin,
        userId,
        rows.map(({ g, titleId, before }) => ({ titleId, at: learnedOrKept(before, g.minutes) })),
      );
