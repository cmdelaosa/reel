...
/** La fecha de última partida que le toca a una fila tras sincronizar.
 *
 *  La aprendida si los minutos han subido respecto a una cifra que YA venía de
 *  Nintendo —comparar contra horas escritas a mano o traídas de Steam diría "ha
 *  jugado" cada vez que alguien las corrige—, y si no, la que ya tenía. Null en
 *  lo que entra nuevo: seguir un juego no es haberlo jugado.
 *
 *  Existe porque el disparador `game_progress_touch` (0075) pone `now()` en
 *  cuanto cambian las horas, y la sincronización escribe esta fecha DETRÁS para
 *  deshacerlo. Sin ella, importar veinte juegos los pondría los veinte arriba
 *  del todo en "A jugar". */
export function learnedOrKept(
  before: { minutes: number | null; source: string | null; playedAt: string | null } | null | undefined,
  incoming: number,
  now = new Date(),
): string | null {
  if (!before) return null;
  const learned = before.source === "nintendo" ? playedAtFor(before.minutes, incoming, now) : null;
  return learned ?? before.playedAt;
}
