...
// 0104, no una comprobación de que TypeScript sabe sumar.
...
/* ── La fecha que se escribe DETRÁS del disparador ──────────────────────── */

const ANTES = "2026-05-01T10:00:00.000Z";
const AHORA = new Date("2026-09-23T18:00:00.000Z");

Deno.test("lo que entra nuevo queda sin fecha: seguir no es haber jugado", () => {
  // Sin esto el disparador de 0075 lo dejaba con hoy, y veinte juegos
  // importados salían los veinte arriba en "A jugar".
  assertEquals(learnedOrKept(null, 900, AHORA), null);
  assertEquals(learnedOrKept(undefined, 900, AHORA), null);
});

Deno.test("si los minutos de Nintendo suben, la fecha es ahora", () => {
  assertEquals(
    learnedOrKept({ minutes: 100, source: "nintendo", playedAt: ANTES }, 130, AHORA),
    AHORA.toISOString(),
  );
});

Deno.test("si no suben, se devuelve la que había", () => {
  assertEquals(learnedOrKept({ minutes: 100, source: "nintendo", playedAt: ANTES }, 100, AHORA), ANTES);
});

Deno.test("una cifra que no era de Nintendo no enseña nada: se queda la fecha de antes", () => {
  // 40 h a mano contra 42 de Nintendo no es una partida: es otra fuente.
  assertEquals(learnedOrKept({ minutes: 2400, source: "manual", playedAt: ANTES }, 2520, AHORA), ANTES);
  assertEquals(learnedOrKept({ minutes: 60, source: "steam", playedAt: null }, 90, AHORA), null);
});
