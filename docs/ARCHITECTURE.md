...
library_entries (
  ...
  minutes_source text,                      -- manual | steam | nintendo (0073; nintendo since 0104)
...
The game imports (Steam 0076/0078, Nintendo 0104) are three tables because the flow is three
...
game_imports (                              -- one per import attempt (was steam_imports)
  id uuid pk, user_id uuid fk,
  provider text not null                    -- 0104. No default on purpose: a forgotten provider
...
game_import_items (                         -- one per game the provider returned
  ...
  external_id text not null,                -- 0104: the provider's own id AS TEXT. Steam writes
...
Nintendo has no login and no library API, so **0104 stores a friend code, not a token**:
